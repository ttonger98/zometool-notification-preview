...
function detail(s,m,{detailLimit,networkFail,imageFail}){
 ...
 const action=m.type==='follow'?'':`<button class="goto-mail" data-action="target" ${m.targetAvailable?'':'disabled'}>${m.targetAvailable?(m.type==='review'?'去修改':'去看看'):'内容已下架'}</button>`;
 return `<div class="detail-head"><h2 class="detail-title">${esc(M.title(s,m))}</h2><div class="detail-time">${time(m.latestAt)}</div></div><div class="meta-row"><span>来自：${esc(m.source)}</span>${thumb}</div><div class="mail-body">${M.unread(m)?'<button class="new-content" data-action="read-new">有新的互动 · 阅读新增内容</button>':''}${esc(M.body(s,m))}</div>${contents}<div class="detail-actions">${action}<button class="delete-mail" data-action="confirm-single">删除消息</button></div>`;
}
