import {firstFollowPopup} from './follow-visual.mjs?v=20260917-feedback1';
import {honorBanner} from './honor-visual.mjs?v=20260917-feedback1';
import * as M from './model.mjs?v=20260917-feedback1';
import {renderCenter,visibleEventIds} from './center-view.mjs?v=20260917-feedback1';
const KEY='zometool-notification-demo-20260917-feedback',LAUNCH=KEY+'-launch';
...
async function goTarget(id,context='main',object){
 await mutate(s=>{const m=M.findMessage(s,id);if(m&&(!M.popupValid(s,m))){toast('该内容已下架或结束');popupId=null;return;}if(networkFail){layer={title:'加载遇到问题',text:'请关闭说明后，在体验设置中恢复正常加载并重试。'};return;}if(m)M.view(s,m.id);targetInfo={returnPage:context==='popup'?'home':page,messageId:id,label:m?M.target(m,context):'造型详情',object:object||m?.object,objects:m?.type==='model_batch'||m?.type==='admission'&&['main','popup'].includes(context)?s.messages.filter(x=>x.type==='admission'&&(M.valid(s,x)||x.id===m.id)).map(x=>x.object):undefined};if(m?.type==='follow_build'&&['main','popup'].includes(context))targetInfo.objects=M.details(s,m).filter(e=>e.work).map(e=>e.work);if(m?.type==='model_batch')targetInfo.objects=[M.OBJECTS.castle,M.OBJECTS.bridge,M.OBJECTS.tower];page='target';settings=false;popupId=null;preview=null;});
}
...
async function act(action,node){
 ...
 if(action==='push'){const pushId=node.dataset.id;const p=state.pushes.find(p=>p.id===pushId);if(!p)return;await mutate(s=>{M.clickPush(s,pushId);settings=false;popupId=null;});await goTarget(p.messageId,'main');return;}
...
await mutate(s=>{s.mode='foreground';if(!popupId)popupId=M.homePopup(s,launch)?.id||null;});
