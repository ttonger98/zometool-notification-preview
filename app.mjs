import {firstFollowPopup} from './follow-visual.mjs?v=20260920-review-sync';
import {honorBanner} from './honor-visual.mjs?v=20260920-review-sync';
import * as M from './model.mjs?v=20260920-review-sync';
import {renderCenter,visibleEventIds} from './center-view.mjs?v=20260920-review-sync';
const demoScene=new URLSearchParams(location.search).get('demo');
const KEY='zometool-notification-demo-20260920-review-sync'+(demoScene?'-'+demoScene:''),LAUNCH=KEY+'-launch';
...
function targetHTML(){
 ...
 return `${header(targetInfo.label,'back-target')}<section class="target"><div class="target-note">业务目标页 · 跳转示意</div><h2>${esc(targetInfo.name||objects[0]?.name||'精彩创作')}</h2>${m&&M.official(m)&&!M.deliveryActive(state,m)?`<p class="body-copy">${['activity','marketing'].includes(m.type)?'活动已结束':'本期内容已结束'}</p>`:''}<div class="gallery">${objects.map(o=>art(o)).join('')}</div>${m?.type==='review'?`<p>${esc(M.body(state,m))}</p>${button('选择清晰的封面','choose-cover','secondary')}${button('重新提交审核','submit-review','gold')}`:['expert_comment','praise'].includes(m?.type)?`<h3>${m.type==='praise'?'夸一夸':'达人评论'}</h3><div class="feedback">${M.details(state,m).map(e=>`<div class="feedback-row" id="feedback-${esc(e.id)}" ${e.id===targetInfo.commentId?'tabindex="-1" aria-label="本条通知最新的夸一夸"':''}>${person(e.actor)}<p>${esc(e.text)}</p></div>`).join('')}</div>`:m?.type==='honor'?'<p>共创达人身份已点亮</p>':m?.type==='growth_star'?honorBanner(m):`<p>${esc(targetInfo.description||'这里展示消息对应的内容与入口。点击返回，可继续查看消息。')}</p>`}<p class="hint">正式业务页面沿用 APP 现有流程。本页用于核对消息的点击去向。</p></section>`;
}
...
async function act(action,node){
 ...
 if(action==='submit-review'){layer={title:'已提交审核',text:'作品进入审核队列。消息中心保留本次调整建议，如仍未通过审核，会收到新的调整建议；审核通过不另发审核结果通知。'};render();return;}
}
...
if(demoScene==='template'||demoScene==='expired-official'){
 ...
 if(M.TYPES[sceneType]&&['center','push','popup'].includes(channel)){
  await mutate(s=>{Object.assign(s,M.createState());launch={device:launch.device,screened:false,shown:null};page=channel==='push'?'outside':'home';popupId=null;settings=false;
   s.mode=channel==='push'?'background':'foreground';
   ...
   M.receive(s,{type:sceneType,object,actor:{id:'scene-user',name:'小宇'},work:sceneType==='follow_build'?{id:'scene-work',name:'蓝色小城',icon:'🌁'}:undefined,popup:true,deliveryExpiresAt:demoScene==='expired-official'?s.now+M.MINUTE:undefined});
