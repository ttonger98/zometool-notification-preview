import {firstFollowPopup} from './follow-visual.mjs?v=20260923-review';
import {honorBanner} from './honor-visual.mjs?v=20260923-review';
import * as M from './model.mjs?v=20260923-review';
import {renderCenter,visibleEventIds} from './center-view.mjs?v=20260923-review';
const demoScene=new URLSearchParams(location.search).get('demo');
const KEY='zometool-notification-demo-20260923-review'+(demoScene?'-'+demoScene:''),LAUNCH=KEY+'-launch';
...
let officialPush=false,officialPopup=false;
...
function deviceMode(s,mode){s.mode=mode;M.setDevice(s,launch.device,{mode});}
...
async function mutate(fn){const work=async()=>{state=load();await fn(state);if(page==='home'&&!document.hidden&&!settings&&!layer&&!preview&&!confirm){const current=M.findMessage(state,popupId||launch.shown);popupId=current?.popupState==='shown'&&current.popupDevice===launch.device?current.id:M.homePopup(state,launch)?.id||null;}persist();render();};return navigator.locks?navigator.locks.request(KEY,work):work();}
...
async function center(cat='all'){await mutate(s=>{M.suspendPopup(s,launch);deviceMode(s,'foreground');page='center';category=cat;order=M.list(s,cat).map(m=>m.id);listLimit=20;selectCurrent(s,order[0]);settings=false;popupId=null;});}
...
function targetHTML(){
 ...
 return `${header(targetInfo.label,'back-target')}<section class="target"><div class="target-note">业务目标页 · 跳转示意</div><h2>${esc(targetInfo.name||objects[0]?.name||'精彩创作')}</h2>${m&&!m.targetAvailable?`<p class="body-copy">${m.object.kind==='model'?'该造型已下架':'该作品已被删除'}</p>`:''}${m&&M.official(m)&&!M.deliveryActive(state,m)?`<p class="body-copy">${['activity','marketing'].includes(m.type)?'活动已结束':'本期内容已结束'}</p>`:''}<div class="gallery">${objects.map(o=>art(o)).join('')}</div>${m?.type==='review'?(m.reviewStatus==='reviewing'?'<p>作品审核中，无需重复提交。审核通过不另发审核结果通知；再次不通过会收到新的调整建议。</p>':`<p>${esc(M.body(state,m))}</p>${button('选择清晰的封面','choose-cover','secondary')}${button('重新提交审核','submit-review','gold')}`):['expert_comment','praise'].includes(m?.type)?`<h3>${m.type==='praise'?'夸一夸':'达人评论'}</h3><div class="feedback">${M.details(state,m).filter(e=>!e.invalidated).map(e=>`<div class="feedback-row" id="feedback-${esc(e.id)}" ${e.id===targetInfo.commentId?'tabindex="-1" aria-label="本条通知最新的夸一夸"':''}>${person(e.actor)}<p>${esc(e.text)}</p></div>`).join('')}</div>`:m?.type==='honor'?'<p>共创达人身份已点亮</p>':m?.type==='growth_star'?honorBanner(m):`<p>${esc(targetInfo.description||'这里展示消息对应的内容与入口。点击返回，可继续查看消息。')}</p>`}<p class="hint">正式业务页面沿用 APP 现有流程。本页用于核对消息的点击去向。</p></section>`;
}
...
function render(){
 const oldList=$('app').querySelector('.mail-list'),scroll=oldList?.scrollTop||0;
 const anchor=oldList&&[...oldList.querySelectorAll('.mail-item')].find(el=>el.getBoundingClientRect().bottom>oldList.getBoundingClientRect().top);
 const anchorId=anchor?.dataset.id,anchorOffset=anchor&&anchor.getBoundingClientRect().top-oldList.getBoundingClientRect().top;
 const oldDetail=$('app').querySelector('.detail-paper'),oldMessageId=oldDetail?.dataset.messageId;
 const detailPositions=[...(oldDetail?.querySelectorAll('.mail-body,.gallery-scroll,.center-feedback')||[])].map(el=>[el.scrollLeft,el.scrollTop]);
 const detailPosition=oldDetail?[oldDetail.scrollLeft,oldDetail.scrollTop]:[0,0];
 if(popupId){const m=M.findMessage(state,popupId);if(!m||m.popupState!=='shown')popupId=null;}
 ...
 const ml=$('app').querySelector('.mail-list');if(ml){ml.scrollTop=scroll;const nextAnchor=[...ml.querySelectorAll('.mail-item')].find(el=>el.dataset.id===anchorId);if(nextAnchor)ml.scrollTop+=nextAnchor.getBoundingClientRect().top-ml.getBoundingClientRect().top-anchorOffset;ml.addEventListener('scroll',()=>{if(ml.scrollTop+ml.clientHeight>=ml.scrollHeight-40&&ordered().length>listLimit){listLimit+=20;render();}},{passive:true});}
 const nextDetail=$('app').querySelector('.detail-paper');if(nextDetail&&nextDetail.dataset.messageId===oldMessageId){[nextDetail.scrollLeft,nextDetail.scrollTop]=detailPosition;[...nextDetail.querySelectorAll('.mail-body,.gallery-scroll,.center-feedback')].forEach((el,i)=>{if(detailPositions[i])[el.scrollLeft,el.scrollTop]=detailPositions[i];});}
...
async function act(action,node){
 ...
 if(action==='home'||action==='foreground'){await mutate(s=>{if(action==='home'&&page!=='home')M.enterHome(launch);page='home';deviceMode(s,'foreground');settings=false;popupId=M.homePopup(s,launch)?.id||null;});return;}
 if(action==='background'){await mutate(s=>{M.suspendPopup(s,launch);popupId=null;page='outside';deviceMode(s,'background');settings=false;M.flushQueue(s);});return;}
 if(action==='close-popup'){await mutate(s=>{M.closePopup(s,popupId,launch);popupId=M.homePopup(s,launch)?.id||null;});return;}
 ...
 if(action==='back-target'){if(targetInfo?.returnPage==='home'){await mutate(s=>{M.enterHome(launch);page='home';popupId=M.homePopup(s,launch)?.id||null;});}else await center(category);return;}
 if(action==='home-target'){targetInfo={returnPage:'home',label:{library:'造型库',works:'作品圈',courses:'课程'}[node.dataset.target],objects:node.dataset.target==='works'?[M.OBJECTS.wheel,M.OBJECTS.rocket]:[M.OBJECTS.castle,M.OBJECTS.bridge,M.OBJECTS.tower]};page='target';render();return;}
 ...
 if(action==='permission'){await mutate(s=>{s.notifications=!(s.devices?.[launch.device]?.notifications??s.notifications);M.setDevice(s,launch.device,{notifications:s.notifications});M.flushQueue(s);});return;}
 if(action==='restart'){await mutate(s=>{M.suspendPopup(s,launch);launch={device:launch.device,screened:false,shown:null};deviceMode(s,'foreground');page='home';settings=false;popupId=M.homePopup(s,launch)?.id||null;});return;}
 ...
 if(['first-follow-push-demo','fans-push-demo','praise-push-demo'].includes(action)){await mutate(s=>{Object.assign(s,M.createState());launch={device:launch.device,screened:false,shown:null};selected=null;order=[];networkFail=false;imageFail=false;popupId=null;page='outside';settings=false;
  if(action==='first-follow-push-demo'){deviceMode(s,'background');M.receive(s,{type:'follow_build',object:M.OBJECTS.bridge,actor:{id:'friend-a',name:'小宇'},work:{id:'first-work',name:'蓝色小城',icon:'🌁'}});}
  else {const t=action==='fans-push-demo'?'follow':'praise',o=t==='follow'?M.OBJECTS.self:M.OBJECTS.wheel;deviceMode(s,'foreground');['小宇','可可','天天'].forEach((name,i)=>{s.now+=M.MINUTE;if(i===2)deviceMode(s,'background');M.receive(s,{type:t,object:o,actor:{id:'friend-'+i,name},text:t==='praise'?['结构很有创意！','颜色搭配真好看！','我也想试试这个造型！'][i]:undefined});});}
 });return;}
 ...
 if(action==='unavailable'){await mutate(s=>{const m=M.findMessage(s,selected)||M.list(s)[0];if(m)M.invalidateTarget(s,m.id);});toast('当前内容已设为下架状态');return;}
 ...
 if(action==='cancel-feedback'){await mutate(s=>{const e=s.events.filter(x=>M.isInteraction(x)&&!x.cancelled&&!x.handled).at(-1)||s.events.filter(x=>M.isInteraction(x)).at(-1);if(e)M.cancelInteraction(s,e.id);});toast('该互动已取消，尚未发送的提醒已停止');return;}
 if(action==='push'){const pushId=node.dataset.id;const p=state.pushes.find(p=>p.id===pushId);if(!p)return;const destination=M.pushDestination(state,p);await mutate(s=>{M.clickPush(s,pushId,launch.device);settings=false;popupId=null;});if(destination.kind==='message'&&!M.list(state).some(m=>m.id===p.messageId)){layer={title:'消息不存在',text:'这条消息已删除或超过保留期。你可以进入消息中心查看其他消息。'};page='home';render();return;}if(destination.kind==='message'){await mutate(s=>{M.suspendPopup(s,launch);page='center';category='feedback';order=M.list(s,category).map(m=>m.id);listLimit=20;selectCurrent(s,p.messageId);});}else await goTarget(p.messageId,'main',undefined,destination);return;}
 if(action==='choose-cover'){toast('封面选择入口 · 示例操作');return;}
 if(action==='submit-review'){if(networkFail){toast('提交失败，请重试');return;}await mutate(s=>{const m=M.findMessage(s,targetInfo?.messageId);if(m)M.resubmit(s,m.object.id);});layer={title:'已提交审核',text:'作品进入审核队列。上一轮通知保留原调整原因，主按钮按作品当前审核状态展示，审核中不能重复提交；再次不通过会收到新的调整建议，审核通过不另发审核结果通知。'};render();return;}
}
...
document.addEventListener('change',e=>{if(e.target.id==='official-push')officialPush=e.target.checked;if(e.target.id==='official-popup')officialPopup=e.target.checked;if(e.target.id==='demo-type'){type=e.target.value;if(!M.allowedKinds(type).includes(M.OBJECTS[objectId].kind))objectId=Object.values(M.OBJECTS).find(o=>M.allowedKinds(type).includes(o.kind)).id;render();}if(e.target.id==='demo-object')objectId=e.target.value;});
...
document.addEventListener('visibilitychange',()=>mutate(s=>{if(document.hidden){M.suspendPopup(s,launch);popupId=null;deviceMode(s,'background');}else deviceMode(s,page==='outside'?'background':'foreground');M.flushQueue(s);}));
window.addEventListener('pagehide',()=>{state=load();M.suspendPopup(state,launch);M.setDevice(state,launch.device,{mode:'background',loggedIn:false});persist();});
await mutate(s=>{M.suspendPopup(s,launch);popupId=null;M.setDevice(s,launch.device,{mode:'foreground',loggedIn:true});deviceMode(s,'foreground');if(!popupId)popupId=M.homePopup(s,launch)?.id||null;});
...
// Deterministic review scenes share the same components and rules as the normal demo.
if(demoScene==='template'||demoScene==='expired-official'){
 const params=new URLSearchParams(location.search),sceneType=demoScene==='expired-official'?'activity':params.get('type'),channel=demoScene==='expired-official'?'center':params.get('channel');
 if(M.TYPES[sceneType]&&['center','push','popup'].includes(channel)){
  await mutate(s=>{Object.assign(s,M.createState());launch={device:launch.device,screened:false,shown:null};page=channel==='push'?'outside':'home';popupId=null;settings=false;
   deviceMode(s,channel==='push'?'background':'foreground');
   const object=['honor','growth_star','follow'].includes(sceneType)?M.OBJECTS.self:['admission','follow_build','model_batch'].includes(sceneType)?M.OBJECTS.bridge:M.OBJECTS.wheel;
   M.receive(s,{type:sceneType,object,actor:{id:'scene-user',name:'小宇'},work:sceneType==='follow_build'?{id:'scene-work',name:'蓝色小城',icon:'🌁'}:undefined,push:true,popup:true,awardPeriod:params.get('period')||undefined,deliveryExpiresAt:demoScene==='expired-official'?s.now+M.MINUTE:undefined});
   if(demoScene==='expired-official')M.advance(s,2*M.MINUTE);
   if(channel==='popup')popupId=M.homePopup(s,launch)?.id||null;
  });
  if(channel==='center')await center();
 }
}
