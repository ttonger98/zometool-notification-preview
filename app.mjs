import {firstFollowPopup} from './follow-visual.mjs?v=20260922-review';
import {honorBanner} from './honor-visual.mjs?v=20260922-review';
import * as M from './model.mjs?v=20260922-review';
import {renderCenter,visibleEventIds} from './center-view.mjs?v=20260922-review';
const demoScene=new URLSearchParams(location.search).get('demo');
const KEY='zometool-notification-demo-20260922-review'+(demoScene?'-'+demoScene:''),LAUNCH=KEY+'-launch';
...
async function mutate(fn){const work=async()=>{state=load();await fn(state);if(page==='home'&&!document.hidden&&!settings&&!layer&&!preview&&!confirm){const current=M.findMessage(state,popupId||launch.shown);popupId=current?.popupState==='shown'&&M.popupValid(state,current)&&current.popupDevice===launch.device?current.id:M.homePopup(state,launch)?.id||null;}persist();render();};return navigator.locks?navigator.locks.request(KEY,work):work();}
