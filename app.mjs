...
function pushCard(p){return `<button class="push-item" data-action="push" data-id="${p.id}"><small>Zometool · ${time(p.at)}</small><strong>${esc(p.title)}</strong><small>点击查看对应内容</small></button>`;}
...
function targetHTML(){
 ...
 return `${header(targetInfo.label,'back-target')}<section class="target"><div class="target-note">业务目标页 · 跳转示意</div><h2>${esc(targetInfo.name||objects[0]?.name||'精彩创作')}</h2><div class="gallery">${objects.map(o=>art(o)).join('')}</div>${m?.type==='review'?`<p>${esc(M.body(state,m))}</p>${button('选择清晰的封面','choose-cover','secondary')}${button('重新提交审核','submit-review','gold')}`:m?.type==='expert_comment'?`<div class="feedback">${M.details(state,m).map(e=>`<div class="feedback-row">${person(e.actor)}<p>${esc(e.text)}</p></div>`).join('')}</div>`:m?.type==='honor'?'<p>🏅 共创达人荣誉 · 已授予</p>':`<p>${esc(targetInfo.description||'这里展示消息对应的内容与入口。点击返回，可继续查看消息。')}</p>`}<p class="hint">正式业务页面沿用 APP 现有流程。本页用于核对消息的点击去向。</p></section>`;
}
function popupHTML(m){return `<div class="overlay"><section class="dialog" role="dialog" aria-modal="true" aria-labelledby="popup-title"><button class="close" data-action="close-popup" aria-label="关闭首页弹窗">×</button><div class="source">Zometool官方</div><div class="art popup-art">${m.type==='admission'?m.object.icon:M.TYPES[m.type][1]}</div><h2 id="popup-title">${m.type==='admission'?'你的造型入库啦！':esc(M.title(state,m))}</h2><p class="body-copy">${esc(M.body(state,m))}</p><div class="dialog-actions">${button('稍后看','close-popup','secondary')}${button(m.type==='admission'?'看看我的造型':m.type==='collection'?'看看作品集':'去看看','popup-target','gold')}</div><p class="hint">这条消息已为你保存在消息中心</p></section></div>`;}
const clock=()=>new Date(state.now+8*M.HOUR).toISOString().slice(0,16).replace('T',' ');
function reviewHTML(){const r=M.round(state,type),c=M.stats(state);return `<div class="overlay"><section class="dialog review-panel" role="dialog" aria-modal="true" aria-label="原型体验设置"><button class="close" data-action="close-settings" aria-label="关闭体验设置">×</button><h2>原型体验设置</h2><p class="hint">2026.09.10 需求同步版 · 示例数据。站外 Push、业务跳转和同浏览器双标签页的状态同步用于评审演示。</p><div class="control-row"><span class="pill">北京时间 ${clock()}</span><span class="pill">${state.mode==='foreground'?'APP前台':'APP后台'}</span><span class="pill">未读 ${c.unread} / 全部 ${c.total}</span></div><div class="review-grid"><section class="review-card"><h3>产生一条新消息</h3><label>消息场景<select id="demo-type">${Object.entries(M.TYPES).map(([key,v])=>`<option value="${key}" ${key===type?'selected':''}>${v[0]}</option>`).join('')}</select></label><label>业务对象<select id="demo-object">${Object.entries(M.OBJECTS).filter(([,o])=>M.allowedKinds(type).includes(o.kind)).map(([key,o])=>`<option value="${key}" ${key===objectId?'selected':''}>${o.name} · ${{model:'造型',work:'作品',remix:'跟拼作品',profile:'个人主页'}[o.kind]}</option>`).join('')}</select></label><label>互动用户（留空自动生成新用户）<input id="demo-actor" value="${esc(actorName)}" placeholder="例如：小宇"></label><div class="control-row">${button('生成新消息','generate','gold')}</div><p>个人结果自动进入首页候选；本演示生成的精选官方通知开启首页提醒。</p></section><section class="review-card"><h3>Push 轮次与体验时钟</h3><p>当前类型：${M.TYPES[type][0]}</p><p>${M.INTERACTIONS.includes(type)?`本轮已用 ${r.used}/2 次 · 剩余 ${r.remaining} 次${r.ends?`<br>窗口截止：${new Date(r.ends+8*M.HOUR).toISOString().slice(11,16)}`:'<br>下一次符合条件的新互动开启计时'}`:'官方与个人结果使用独立发送序列'}</p><div class="control-row">${button('+1分钟','advance','secondary','data-minutes="1"')}${button('+5分钟','advance','secondary','data-minutes="5"')}${button('+2小时','advance','secondary','data-minutes="120"')}</div><div class="control-row">${button('下一日07:00','time-seven','secondary')}${button(new Date(state.now+8*M.HOUR).getUTCHours()<21?'今天21:00':'下一日21:00','time-night','secondary')}</div><div class="control-row">${button(state.mode==='foreground'?'切到后台':'回到前台',state.mode==='foreground'?'background':'foreground')}${button(state.notifications?'通知许可：开启':'通知许可：关闭','permission','secondary')}</div><p>每次互动 Push 由最新互动触发；窗口结束后恢复额度。全局发送间隔一分钟，发送时段07:00—21:00。</p></section><section class="review-card"><h3>首页与状态联动</h3><p>本次启动：${launch.shown?'已展示一条提醒':launch.screened?'已完成首页筛选':'等待首页筛选'}。<br>关闭提醒后，本条消息通过消息中心回看。</p><div class="control-row">${button('重启APP','restart','gold')}${button('打开消息中心','center')}${button('返回首页','home','secondary')}</div><div class="control-row">${button('双设备演示说明','devices','secondary')}${button('空消息场景','empty','secondary')}${button('恢复示例','reset','secondary')}</div><p class="hint">“重启APP”模拟新的进程启动；后台返回沿用当前启动记录。</p></section><section class="review-card"><h3>边界状态</h3><div class="control-row">${button(networkFail?'详情加载：失败':'详情加载：正常','network','secondary')}${button(imageFail?'图片：默认图':'图片：正常','images','secondary')}${button('当前内容下架','unavailable','secondary')}${button('90天后','expire','secondary')}${button('重复回调','duplicate','secondary')}</div><p>失败时保留阅读状态；图片异常展示默认图；过期与下架即时更新候选资格。</p><h3 style="margin-top:15px">站外 Push 记录（${state.pushes.length}）</h3><div class="push-feed">${state.pushes.slice(-8).reverse().map(pushCard).join('')||'<p>等待新的有效事件</p>'}</div></section></div></section></div>`;}
function otherLayer(){
...
function render(){
 ...
 if(popupId){const m=M.findMessage(state,popupId);if(!m||m.popupState==='completed'||!M.valid(state,m))popupId=null;}
...
async function goTarget(id,context='main',object){
 await mutate(s=>{const m=M.findMessage(s,id);if(m&&(!M.valid(s,m)||!m.targetAvailable)){toast('该内容已下架或结束');popupId=null;return;}if(networkFail){layer={title:'加载遇到问题',text:'请关闭说明后，在体验设置中恢复正常加载并重试。'};return;}if(m)M.view(s,m.id);targetInfo={messageId:id,label:m?M.target(m,context):'造型详情',object:object||m?.object,objects:m?.type==='model_batch'||m?.type==='admission'&&context==='main'?s.messages.filter(x=>x.type==='admission'&&M.valid(s,x)).map(x=>x.object):undefined};if(m?.type==='follow_build'&&context==='main')targetInfo.objects=M.details(s,m).filter(e=>e.work).map(e=>e.work);if(m?.type==='model_batch')targetInfo.objects=[M.OBJECTS.castle,M.OBJECTS.bridge,M.OBJECTS.tower];page='target';settings=false;popupId=null;preview=null;});
}
...
async function act(action,node){
 ...
 if(action==='close-settings'){settings=false;render();return;}
 ...
 if(action==='home'||action==='foreground'){await mutate(s=>{page='home';s.mode='foreground';settings=false;});return;}
 if(action==='background'){await mutate(s=>{page='outside';s.mode='background';settings=false;M.flushQueue(s);});return;}
 if(action==='close-popup'){await mutate(s=>{M.closePopup(s,popupId);popupId=null;});return;}
 ...
 if(action==='back-target'){await center(category);return;}
 ...
 if(action==='restart'){await mutate(s=>{launch={device:launch.device,screened:false,shown:null};s.mode='foreground';page='home';settings=false;popupId=M.homePopup(s,launch)?.id||null;});return;}
 ...
 if(action==='devices'){layer={title:'双设备状态演示',text:'在同一浏览器打开第二个体验窗口。两个窗口共享示例账号的消息状态；查看、删除与关闭首页提醒会同步。每个窗口分别保存当次启动的展示额度。',link:true};settings=false;render();return;}
 ...
 if(action==='push'){const pushId=node.dataset.id;await mutate(s=>{const p=s.pushes.find(p=>p.id===pushId),m=p&&M.findMessage(s,p.messageId);if(!m||!M.valid(s,m)){toast('这条消息已结束，其他消息可在消息中心查看');return;}s.mode='foreground';settings=false;page='center';category='all';order=M.list(s).map(m=>m.id);selectCurrent(s,p.messageId);});return;}
...
await mutate(s=>{s.mode='foreground';if(!launch.screened)popupId=M.homePopup(s,launch)?.id||null;});
